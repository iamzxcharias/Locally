...
public class ParticipationJpaRepository implements PanacheRepositoryBase<ParticipationJpaEntity, UUID> {
    ...
    private QueryParts buildQuery(UUID userId, UUID eventId, ParticipationStatus status) {
        ...
        if (userId != null) { q.append(" and userId = :userId"); p.put("userId", userId); }
        if (eventId != null) { q.append(" and eventId = :eventId"); p.put("eventId", eventId); }
        if (status != null) { q.append(" and status = :status"); p.put("status", status); }
